...
Your task is to generate a structured presentation script for a {{DURATION_MINUTES}}-minute presentation tailored to this audience.

Rules:
- Output ONLY valid JSON matching the schema below. No markdown fencing, no commentary.
- Target a total duration of approximately {{TARGET_SECONDS}} seconds ({{DURATION_MINUTES}} minutes). The sum of all slide durationSeconds must equal totalDurationSeconds.
